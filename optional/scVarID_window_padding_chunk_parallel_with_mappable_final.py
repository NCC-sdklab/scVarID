...
scVarID pipeline:
 ...
 - minimal handle_insertion/deletion
...
def parse_arguments():
    parser= argparse.ArgumentParser(description='scVarID chunk parallel + mappable + minimal Indel, final logs.')
    parser.add_argument('--variant-files', nargs='+', required=True,
                        help='Pairs: sample1 file1.vcf sample2 file2.vcf ...')
...
############################################################################
# 3. Read fetch + PyRanges => mappable
############################################################################
...
def process_one_chunk_and_dump(
    ...
    # 4) classification => minimal Indel handle => store
