# scVarID_window_padding_chunk_parallel_final_integrated.py

#!/usr/bin/env python
# -*- coding: utf-8 -*-

"""
scVarID Pipeline - Integrated Version
- Chunk-level Parallel Processing
- Region-based VCF/BAM Fetch
- Accurate Read Parsing and Classification Logic
- Manage Temporary Intermediate Files and Save Final HDF5 Outputs
- Handle Missing Barcodes and Improve 'Unknown' Classification
"""
...
def extract_reads_with_padding(bam_path, chrom, start, end, padding=5000):
    ...
    # Log the number of reads without CB tag
    missing_cb = df_reads['Cell_Barcode'].isnull().sum()
    if missing_cb > 0:
        logging.warning(f"{missing_cb} reads without CB tag found in {chrom}:{start}-{end}")
...
def process_one_chunk_and_dump(
    ...
    if not union_variants:
        logging.info(f"[CHUNK] {chunk_str} has no variants. Skipping.")
        return None
    ...
    if df_reads.empty:
        logging.info(f"[CHUNK] {chunk_str} has no reads. Skipping.")
        return None
    ...
    # 5. Classify reads
    if not read_uniq_list:
        logging.info(f"[CHUNK] {chunk_str} has no unique reads to classify.")
        return None
    ...
    refC, altC, missC, unkC = process_bam_data_parallel(
        ...
        final_variants,
        read_mapping,
        num_cores=1,  # Parallelization already handled at pipeline level
