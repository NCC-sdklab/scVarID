...
def extract_reads_info_region(bam_path, chrom, start, end, barcode_tag):
    ...
    if total_barcodes == 0:
        logging.warning("No barcodes were extracted from the reads. Please check the barcode tag and BAM file.")
    return df
...
def check_num_cores(num_cores):
    """
    Ensures that the number of specified cores does not exceed available cores.
    If it does, it adjusts to the maximum available and logs a warning.
    """
