...
scVarID pipeline:
 ...
 - Enhanced handle_insertion/deletion for accurate classification
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='scVarID chunk parallel + mappable + enhanced Indel handling.')
    parser.add_argument('--variant-files', nargs='+', required=True,
                        help='Pairs: sample1 file1.vcf.gz sample2 file2.vcf.gz ...')
    ...
    parser.add_argument('--num-cores', type=int, default=4, help='Number of chunk parallel processes')
...
def handle_insertion(sequences, positions, ops, pos_, ref_, alt_, rname, bc):
    """
    Handles insertion classification.
    If the read has the inserted bases matching the alt, classify as 'alt'.
    Else, classify as 'missing'.
    """
    try:
        idx = positions.index(pos_)
    except ValueError:
        return 'missing', bc, pos_, rname

    # Insertion should follow the reference base
    # Alt is ref + inserted bases
    inserted_bases = ''.join(sequences[idx+1:idx+1+len(alt_)-len(ref_)]) if len(alt_) > len(ref_) else ''
...
def handle_deletion(sequences, positions, ops, pos_, ref_, alt_, rname, bc):
    """
    Handles deletion classification.
    If the read has the deletion matching the ref, classify as 'alt'.
    Else, classify as 'missing'.
    """
    try:
        idx = positions.index(pos_)
    ...
    # Deletion spans len(ref_) - len(alt_) bases
    del_len = len(ref_) - len(alt_)
    if del_len <= 0:
        return 'unknown', bc, pos_, rname

    # Check if the next del_len positions are deletions ('D')
    if all(op == 'D' for op in ops[idx:idx + del_len]):
        return 'alt', bc, pos_, rname
    else:
        return 'missing', bc, pos_, rname
...
def classify_chunk_reads_bam(
    ...
    """
    Classify reads for variants in the chunk.
    """
        ...
        for read in bam.fetch(until_eof=True):
            ...
            if rnm in read_mapping:
                if c_ < len(read_mapping[rnm]):
                    read_unique = read_mapping[rnm][c_]
                else:
                    read_unique = f"{rnm}_{c_}"
            else:
                read_unique = f"{rnm}_{c_}"
...
def process_one_chunk_and_dump(
    ...
    # 4) classification => enhanced Indel handling => store
