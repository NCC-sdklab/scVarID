...
def handle_insertion(sequences, positions, operations, pos_, ref_, alt_, rname, bc):
    try:
        ...
        if (all(op == '=' for op in operations[pos_index : pos_index + len(ref_)]) and
            sequences[pos_index + len(ref_) : pos_index + len(alt_) - len(ref_) + pos_index] == list(alt_[len(ref_):]) and
            all(op == 'I' for op in operations[pos_index + len(ref_) : pos_index + len(alt_) - len(ref_) + pos_index]) and
            (pos_index + len(alt_) - len(ref_) + pos_index < len(operations) and
             operations[pos_index + len(alt_) - len(ref_) + pos_index] == '=')):
            return 'alt', bc, pos_, rname
...
def handle_deletion(sequences, positions, operations, pos_, ref_, alt_, rname, bc):
    try:
        pos_index = positions.index(pos_)
        ...
        # Check for alt status
        if (all(op == '=' for op in operations[pos_index : pos_index + len(alt_)]) and
            all(seq == '-' for seq in sequences[pos_index + len(alt_) : pos_index + len(ref_) - len(alt_) + pos_index]) and
            all(op == 'D' for op in operations[pos_index + len(alt_) : pos_index + len(ref_) - len(alt_) + pos_index]) and
            (pos_index + len(ref_) - len(alt_) + pos_index < len(operations) and
             operations[pos_index + len(ref_) - len(alt_) + pos_index] == '=')):
            return 'alt', bc, pos_, rname
...
def process_one_chunk_and_dump(
    ...
    refC, altC, missC, unkC = process_bam_data_parallel(
        ...
        df_reads2.set_index("Read_Name")["Read_Unique_Name"].to_dict(),
