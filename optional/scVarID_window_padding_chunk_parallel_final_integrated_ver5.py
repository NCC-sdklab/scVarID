...
def handle_insertion(sequences, positions, operations, pos, ref, alt, read_name, cell_barcode):
    ...
    try:
        ...
        # Check if the insertion occurs after the reference base
        # For insertions, alt length > ref length
        if len(alt) > len(ref):
            insertion_length = len(alt) - len(ref)
            # 삽입된 시퀀스 추출
            insertion_seq = ''.join(sequences[pos_index + 1: pos_index + 1 + insertion_length])
            expected_insertion = alt[len(ref):]
            if insertion_seq == expected_insertion:
                logging.debug(f"Insertion matched: {insertion_seq} == {expected_insertion}")
                return 'alt', cell_barcode, pos, read_name
            else:
                logging.debug(f"Insertion mismatch: {insertion_seq} != {expected_insertion}")
                return 'unknown', cell_barcode, pos, read_name
        else:
            return 'unknown', cell_barcode, pos, read_name
    except (ValueError, IndexError):
        logging.debug(f"Insertion handling failed at pos {pos}")
        return 'unknown', cell_barcode, pos, read_name
...
def handle_deletion(sequences, positions, operations, pos, ref, alt, read_name, cell_barcode):
    ...
    try:
        pos_index = positions.index(pos)
        # Check if the deletion spans the reference length
        if len(ref) > len(alt):
            deletion_length = len(ref) - len(alt)
            # 삭제된 위치의 시퀀스가 '-'인지 확인
            deleted_seq = ''.join(sequences[pos_index + 1: pos_index + 1 + deletion_length])
            if all(seq == '-' for seq in deleted_seq):
                logging.debug(f"Deletion matched: {deleted_seq} are all '-'")
                return 'alt', cell_barcode, pos, read_name
            else:
                logging.debug(f"Deletion mismatch: {deleted_seq} not all '-'")
                return 'unknown', cell_barcode, pos, read_name
        else:
            return 'unknown', cell_barcode, pos, read_name
    except (ValueError, IndexError):
        logging.debug(f"Deletion handling failed at pos {pos}")
        return 'unknown', cell_barcode, pos, read_name
...
def process_one_chunk_and_dump(
    ...
    refC, altC, missC, unkC = process_bam_data_parallel(
        ...
        final_variants,
        read_mapping,
        num_cores=1,  # Already parallelized at the pipeline level
