...
def handle_insertion(sequences, positions, operations, pos_, ref_, alt_, read_name, cell_barcode):
    try:
        ...
        # Sequences from pos_index + len(ref_) to pos_index + len(alt_)
        # Operations from pos_index + len(ref_) to pos_index + len(alt_)
        # Ensure that the inserted bases match and operations are 'I'
        ...
        # Using inequality: check if inserted_bases >= expected_insert
        # Adjust based on specific requirements; here, it's assumed that if inserted_bases lexicographically
        # equal or greater than expected_insert, it's considered 'alt'
...
def handle_deletion(sequences, positions, operations, pos_, ref_, alt_, read_name, cell_barcode):
    try:
        ...
        # Check for alt status
        # Deleted bases should match expected deletion
        ...
        # Using inequality: check if deleted_bases >= expected_delete
...
############################################################################
# 5. process_bam_data_parallel and process_read_chunk
############################################################################
