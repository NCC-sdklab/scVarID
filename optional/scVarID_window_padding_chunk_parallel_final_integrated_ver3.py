...
def handle_insertion(sequences, positions, operations, pos, ref, alt, read_name, cell_barcode):
    ...
    try:
        ...
        if len(alt) > len(ref):
            insertion_seq = ''.join(sequences[pos_index + 1: pos_index + 1 + (len(alt) - len(ref))])
...
def handle_deletion(sequences, positions, operations, pos, ref, alt, read_name, cell_barcode):
    ...
    try:
        ...
        if operations[pos_index] == 'D':
            # For deletions, ref length > alt length
            # Confirm that the deletion is correctly represented
            deleted_seq = ''.join(sequences[pos_index: pos_index + (len(ref) - len(alt))])
            # In read sequences, deletions are represented by '-'
            if all(seq == '-' for seq in sequences[pos_index: pos_index + (len(ref) - len(alt))]):
                return 'alt', cell_barcode, pos, read_name
            else:
                return 'unknown', cell_barcode, pos, read_name
        else:
