...
scVarID pipeline:
 ...
 - final HDF5 output
Requires: pyranges, pysam, joblib, pandas, scipy, h5py
"""
...
def extract_reads_info_region(bam_path, chrom, start, end):
    ...
    with pysam.AlignmentFile(bam_path,"rb") as bam:
        # fetch is 0-based, end is exclusive => pass (start-1, end)
        for read in bam.fetch(chrom, start-1, end):
            ...
            count_= read_name_counts[rname]
            read_unique= f"{rname}_{count_}"
            read_name_counts[rname]+=1

            # compute end pos with cigar
...
def annotate_reads_with_variants_pyranges(df_reads, union_variants):
    """
    PyRanges overlap => each read => which variants => add 'VINDEX' col
    Also logs # mappable reads
    """
    import pyranges as pr
    ...
    # join => find overlap
    overlap= rd_pr.join(var_pr)  # => overlapping reads x variants
    overlap_df= overlap.df
    # group by read => all VINDEX
    read_vindex= overlap_df.groupby("Read_Unique_Name")["VINDEX"].apply(lambda x: ",".join(x)).reset_index()

    df_reads= df_reads.merge(read_vindex, on="Read_Unique_Name", how="left")
    df_reads["VINDEX"]= df_reads["VINDEX"].fillna("")

    # mappable read => VINDEX != ""
    total_= len(df_reads)
...
def annotate_variants_with_reads_pyranges(union_variants, df_reads):
    """
    PyRanges overlap => each variant => which reads => add 'Read_Names'
    also logs # mappable variants
    """
    import pyranges as pr
    df_var= pd.DataFrame(union_variants).copy()
    if df_var.empty:
        for var_ in union_variants:
            var_["Read_Names"]=""
...
def process_reads_and_variants_pyranges(df_reads, union_variants):
    """
    1) annotate reads => VINDEX => logs # mappable reads
    2) annotate variants => Read_Names => logs # mappable variants
    3) return df_reads, updated_union_variants
    """
...
def prime_read_for_classify(read):
    ...
    note: similar to prime_read above but let's keep separate to avoid confusion
    """
    rname= read.query_name
    try:
        bc= read.get_tag("CB")
...
def classify_chunk_reads_bam(
    ...
    """
    we do a simple approach: we have read_uniq => read_name_counts
    but we'll just do single-thread or minimal
    For chunk-level, num_cores can be used but be mindful of nested parallel
    """
    # 1) build variants_dict
    variants_dict= create_variants_dict(updated_union_variants)

    # 2) selected read uniques
...
def process_one_chunk_and_dump(
    ...
    # 3) PyRanges overlap => mappable reads
    #    => mappable variants
    ...
    #    (here we do single-thread to avoid nested parallel warning)
    ...
    chunk_data= {
      "union_variants": updated_union_variants,  # already annotated with 'Read_Names' if needed
      "ref": refC,
      "alt": altC,
      "missing": missC,
      "unknown": unkC
...
def main():
    ...
    chunk_files= Parallel(n_jobs=num_cores)(
        delayed(process_one_chunk_and_dump)(
            ...
            padding= pad,
            compute_missing_unknown= compute_missing_unknown,
            num_cores=1,  # chunk-level => single inside chunk
            tmp_dir= tmp_dir
        )
        for c in all_chunks
    )
